...
const HealthPage = props => (
    <>
      ...
      <div>
      <h5 className="bg-light">ఆరోగ్యం</h5>
...
HealthPage.getInitialProps = async context => {
  ...
  const health = await client.getEntries({ content_type: "health", 'fields.teluguDescription[exists]': true}).then((response) => response.items);
